...
# Explore (Mark the regions with the maximum/minimum values)



```{r, fig.width=10,fig.height=6}
top_area <- NGA_wp %>%
  filter(wp_nonfunctional == max(wp_nonfunctional, na.rm = TRUE))

bottom_area <- NGA_wp %>%
  filter(wp_nonfunctional == min(wp_nonfunctional, na.rm = TRUE))

tmap_mode("plot")
tm_shape(NGA_wp) +
  tm_polygons("wp_nonfunctional",
              palette = "Blues",
              title = "Non-functional Water Points") +
  tm_shape(top_area) +
  tm_symbols(shape = 21, col = "red", size = 0.5, border.col = "black") +
  tm_text("lga_name", size = 0.6, col = "red", bg.color = "white") +
  tm_shape(bottom_area) +
  tm_symbols(shape = 21, col = "green", size = 0.5, border.col = "black") +
  tm_text("lga_name", size = 0.6, col = "green", bg.color = "white") +
  tm_layout(main.title = "Top & Bottom LGA by Non-functional Water Points")
```
