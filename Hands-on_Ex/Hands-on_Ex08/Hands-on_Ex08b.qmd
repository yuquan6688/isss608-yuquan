...
In this hands-on exercise, I will learn by using appropriate R packages:
...
![](images/clipboard-3842893779.png)
...
![](images/clipboard-2211151282.png)

I display the basic information of the newly created sgpools_sf by using the code chunk below.
...
Before I end the session, it is wiser to switch tmap’s Viewer back to plot mode by using the code chunk below.
...
tm_shape(sgpools_sf) +
  tm_bubbles(size = "Gp1Gp2 Winnings",
             fill = "OUTLET TYPE",
             col = "black") +
  tm_layout(title = "Winnings Distribution over Base Map")
```

## 2 Different shapes represent different types
...
tm_shape(sgpools_sf) +
  tm_bubbles(size = "Gp1Gp2 Winnings",
             ...
             shape = "OUTLET TYPE",
             col = "black") +
  tm_layout(title = "Symbol Shape by Outlet Type",
            legend.outside = TRUE)
```
